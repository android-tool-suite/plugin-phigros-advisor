(() => {
  ...
  function setLoading(value){
    ...
    if(!value&&pendingPage){pendingPage=false;queueMicrotask(loadPage);}
  }
  const message=createFeedback(ui.message);
  let localData, initializing = true, pageLoad, pendingPage = false;
  function createLocalData(){return phigrosInitialLoad.createLoader(ats.readDataset,async(id,bytes)=>{
    if(id==='profiles')state.profiles=bytes?JSON.parse(text(bytes)):{formatVersion:1,profiles:[],selectedProfile:null,widget:{}};
    else if(id==='song-catalog')state.catalog=bytes?parseCatalog(text(bytes)):[];
    else if(id==='analysis-data'){
      state.analysisFiles=new Map();state.save=null;state.snapshot=null;state.pushTargets={};state.pushStatus='loading';state.timeline=[];
      if(bytes)await loadAnalysis(bytes);
    }else if(id==='session-tokens'){
      state.tokens=bytes?JSON.parse(text(bytes)):{formatVersion:1,tokens:[]};
  ...
  function pushLabel(id,accuracy){const target=state.pushTargets[id];if(target)return target.targetAccuracy!=null?Number(target.targetAccuracy).toFixed(4)+'%':accuracy>=100?'已达 Phi':'无法推分';return state.pushStatus==='error'?'计算失败':'计算中…';}
  function updatePushLabels(){document.querySelectorAll('[data-push-identity]').forEach(node=>{node.textContent=pushLabel(node.dataset.pushIdentity,Number(node.dataset.accuracy));});}
  function startPushTargets(){
    const save=state.save,profileId=state.profiles.selectedProfile;
    state.pushTargets={};state.pushStatus='loading';
    state.pushPromise=phigrosPushTargets.get(save.records||[],profileId,save.pushTargetCache).then(result=>{
      if(state.save!==save||state.profiles.selectedProfile!==profileId)return null;
      state.pushTargets=result.cache.targets;state.pushStatus='ready';updatePushLabels();
      if(result.cacheError)message('推分已计算，但缓存保存失败，下次将重新计算','danger');
      return result.cache;
    }).catch(error=>{if(state.save===save&&state.profiles.selectedProfile===profileId){state.pushStatus='error';updatePushLabels();message(error.message||'推分计算失败，请刷新重试','danger');}return null;});
  }
  async function commitCloudSave(next,profileId){
    const cache=await phigrosPushTargets.prepare(next.records||[],profileId);
    next={...next,pushTargetCache:cache};
    const scope=safeScope(profileId),files=new Map(state.analysisFiles),previousBytes=files.get(`saves/save-${scope}.json`),timelineBytes=files.get(`timeline-${scope}.json`),previous=previousBytes?JSON.parse(text(previousBytes)):null,oldTimeline=timelineBytes?JSON.parse(text(timelineBytes)):[],snapshot=phigrosRks.calculate(next.records||[]),event=historyEvent(previous,next,snapshot),timeline=phigrosPresentation.appendHistory(oldTimeline,event);
    files.set(`saves/save-${scope}.json`,encode(next));files.set(`last-records-${scope}.json`,encode(next.records||[]));files.set(`timeline-${scope}.json`,encode(timeline));
    await ats.writeDataset('analysis-data',phigrosZip.write(files));state.analysisFiles=files;
    if(profileId===state.profiles.selectedProfile){state.profiles.widget={rks:snapshot.overall,count:next.records.length,player:next.profile.playerId};await ats.writeDataset('profiles',encode(state.profiles));state.save=next;state.snapshot=snapshot;state.pushTargets=cache.targets;state.pushStatus='ready';state.pushPromise=Promise.resolve(cache);state.timeline=timeline;}
    return snapshot;
  }
  async function consumePendingSync(profileId){const pending=await ats.call('storage.kv.get',{key:'pending-cloud-sync'});if(!pending.found||pending.value?.profileId!==profileId)return false;await Promise.all([localData.ensure('analysis-data'),localData.ensure('song-catalog')]);const value=pending.value||{};try{const bytes=await ats.readBlob(value.blobId,8*1024*1024);if(!bytes)throw new Error('后台同步文件不存在');const next=await phigrosSaveParser.parse(bytes,value.player||{},value.saveInfo||{},state.catalog);await commitCloudSave(next,value.profileId);message('已应用后台同步结果','success');}finally{if(value.blobId)await ats.call('storage.blob.delete',{id:value.blobId}).catch(()=>{});await ats.call('storage.kv.delete',{key:'pending-cloud-sync'}).catch(()=>{});}return true;}
  async function syncNow(){const profile=selected();if(!profile)throw new Error('请先添加并选择账号');if(await consumePendingSync(profile.id)){render();return;}await Promise.all([localData.ensure('session-tokens'),localData.ensure('song-catalog'),localData.ensure('analysis-data')]);const secret=await ats.call('storage.secret.get',{datasetId:'session-tokens',key:`token:${profile.id}`});if(!secret.found)throw new Error('当前账号缺少 SessionToken');const token=String(secret.value||'');const config=serverConfig(profile.server);const headers={'X-LC-Id':config.appId,'X-LC-Key':config.appKey,'X-LC-Session':token,'User-Agent':'LeanCloud-CSharp-SDK/1.0.3'};const player=await requestJson(`${config.base}/users/me`,{headers});if(!player.objectId)throw new Error('SessionToken 无法读取玩家信息');const where=encodeURIComponent(JSON.stringify({user:{__type:'Pointer',className:'_User',objectId:player.objectId}}));const saves=await requestJson(`${config.base}/gamesaves?skip=0&limit=100&where=${where}&include=cover,gameFile`,{headers});const candidates=(saves.results||[]).filter(item=>item.gameFile?.url);if(!candidates.length)throw new Error('没有找到可下载的云存档');candidates.sort((a,b)=>String(b.modifiedAt?.iso||b.updatedAt||'').localeCompare(String(a.modifiedAt?.iso||a.updatedAt||'')));const saveInfo=candidates[0],download=await ats.request(saveInfo.gameFile.url,{deadlineMs:60000});if(download.status<200||download.status>=300)throw new Error(`下载云存档失败（HTTP ${download.status}）`);const next=await phigrosSaveParser.parse(download.bytes,player,saveInfo,state.catalog);await commitCloudSave(next,profile.id);render();}
  ...
  async function deleteToken(id){if(!id)return;await localData.ensure('session-tokens');state.profiles.profiles=state.profiles.profiles.filter(item=>item.id!==id);state.tokens.tokens=(state.tokens.tokens||[]).filter(item=>item.id!==id);state.pendingDelete=null;if(state.profiles.selectedProfile===id)state.profiles.selectedProfile=state.profiles.profiles[0]?.id||null;await ats.call('storage.secret.delete',{datasetId:'session-tokens',key:`token:${id}`});await Promise.all([ats.writeDataset('profiles',encode(state.profiles)),ats.writeDataset('session-tokens',encode(state.tokens))]);await load();}
  ...
  async function exportImage(kind){await runAction(async()=>{
    ...
    await state.pushPromise;if(state.pushStatus!=='ready')throw new Error('推分计算尚未完成，请刷新后重试');
