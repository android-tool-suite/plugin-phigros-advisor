...
(function (root) {
  function createLoader(read, apply) {
    const pending = new Map(), loaded = new Set(), failed = new Set();
    ...
    function ensure(id) {
      ...
      if (!pending.has(id)) {failed.delete(id);pending.set(id, Promise.resolve().then(() => read(id, limits[id]))
        .then(value => apply(id, value)).then(() => { loaded.add(id); })
        .catch(error => {failed.add(id);throw error;})
        .finally(() => pending.delete(id)));}
      return pending.get(id);
    }
    return {ensure, has:id=>loaded.has(id), failed:id=>failed.has(id), page:page=>ensure(page==='定数表'?'song-catalog':'analysis-data')};
  }
